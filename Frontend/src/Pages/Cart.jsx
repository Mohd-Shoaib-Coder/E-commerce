...
const Cart = ({ setCartUpdated }) => {
  ...
  useEffect(() => {
    const getCart = async () => {
      try {
        const response = await fetch("https://e-commerce-backend-gshd.onrender.com/verify", {
          method: "GET",
          credentials: "include",
        });
  ...
  const payment = async () => {
    const response = await fetch("https://e-commerce-backend-gshd.onrender.com/payment", {
      method: "POST",
      headers: {
